...
def fetch(uri):
    ...
    print"Tiempo recorrido", n
    print"-- Url Traida - fuera del TRY --"
    data = response.content

    return data

def average(nums):
    print("FUNCION AVERAGE")
    return sum(nums) * 1.0 / len(nums)

def stddev(nums):
    print("FUNCION STDDEV")
    avg = average(nums)
    variance = map(lambda x: (x - avg)**2, nums)
    avg_var = average(variance)
    print("FIN DE FUNCION STDDEV")
    return math.sqrt(avg_var)

def get_load(instance,service):
    print("FUNCION GET_LOAD")
        ...
        if channel == 'tcp' and decoder == 'http' and encoder == 'xml2' and protocol == 'httpxml':
            ...
            for address in addresses:
                if address.xpath('./type/text()')[0] == '0' and bindtype == expected_bindtype:
                    ...
                    print url
                    print("ANTES DE TRAER LA OTRA URL")
                    response = fetch(url)
                    print("DESPUES DEL FETCH")
                    root = etree.fromstring(response)
                    load = 0
                    dsload = 0
                    try:
                        print("Fin de funcion GET LOAD - 1 try")
...
print("Expected bindtype: ", expected_bindtype)
