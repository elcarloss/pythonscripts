...
if __name__ == "__main__":
    ...
    CSV_F = csv.reader(F)
    for row in CSV_F:
        portservice = PortService(row[2])
        if portservice.has_host(row[0]):
            print"Hostname %s  is already added to pas in %s environment." %(row[0], row[2])
        else:
            try:
                portservice.add_host(row[0], row[1])
                print "                           *********** SUCCESS **************\n"
                print"Hostname %s in environment %s with internal ip %s has been created correctly" \
	             %(row[0], row[2], row[1])
