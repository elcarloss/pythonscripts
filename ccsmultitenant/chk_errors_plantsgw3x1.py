...
def notifyissues(sc):
    msg_issues = "!!ALERT!! plantsvszombies-gw3-xone errors in the {database,system,slivers,user,redis} blaze components are incremeting above the treshold !!ALERT!!"
...
def notifyFlatbehavior(sc):
    msg_issues = "!!WARNING!! plantsvszombies-gw3-xone Errors is flat with None values !!WARNING!!"
    sc.api_call('chat.postMessage', channel=CHANNEL, text=msg_issues, username='log_error_alert', icon_emoji=':siren:')
...
def fetch(Timeout):
    ...
    url = "http://gs-prod-bg-web.bio-iad.ea.com/render/?format=json&target=nonNegativeDerivative(sumSeriesWithWildcards(aliasByNode(eadp.gos.blaze.prod.plantsvszombies-gw3one.*.logger.{database,system,slivers,user,redis}.{ERR,FAIL})))"
