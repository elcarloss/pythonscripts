...
def notifyissues(sc):
    msg_issues = "!!ALERT!! fifa-2020-ps4 errors in the {database,system,slivers,user,redis} blaze components are incrementing above the normal treshold !!ALERT!!"
...
def notifyFlatbehavior(sc):
    msg_issues = "!!WARNING!! fifa-2020-ps4 error number is flat with None values !!WARNING!!"
    sc.api_call('chat.postMessage', channel=CHANNEL, text=msg_issues, username='log_error_alert', icon_emoji=':siren:')
...
#    url = "http://gs-prod-bg-web.bio-iad.ea.com/render/?format=json&target=nonNegativeDerivative(sumSeriesWithWildcards(aliasByNode(eadp.gos.blaze.prod.fifa-2020-xone.*.logger.*.{ERR,FAIL})))"

    url = "http://gs-prod-bg-web.bio-iad.ea.com/render/?format=json&target=nonNegativeDerivative(sumSeriesWithWildcards(aliasByNode(eadp.gos.blaze.prod.fifa-2020-ps4.*.logger.{database,slivers,user,redis}.{ERR,FAIL})))"
...
if 'null' not in ENDPOINTDATA:
    if float(ENDPOINTDATA[0]) < float(TRESHOLD) and float(ENDPOINTDATA[1]) < float(TRESHOLD):
        plugin_exit(STATUS_OK, 'Fifa-2020-ps4 Errors number below treshold', TRESHOLD)       
    elif float(ENDPOINTDATA[0]) > float(TRESHOLD) or float(ENDPOINTDATA[1]) > float(TRESHOLD):
        notifyissues(sc)
        plugin_exit(STATUS_CRIT, 'Fifa-ps4-ps4 Errors number is above treshold', TRESHOLD)
    elif all("None" in ENDPOINTDATA for x in ENDPOINTDATA): 
        notifyFlatbehavior(sc)
        plugin_exit(STATUS_WARN, 'Fifa-2020-ps4 data is flat with None Values', '')
