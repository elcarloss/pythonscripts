...
def notifyissues(sc):
    msg_issues = "!!!ALERT!!! madden-2020-ps4 errors in the {database,system,slivers,user,redis} blaze components are incrementing above the treshold_ !!!ALERT!!!"
...
def notifyFlatbehavior(sc):
    msg_issues = "!!WARNING!! madden-2020-ps4 ERROR rate is flat with None values !!WARNING!!"
...
def fetch(Timeout):
    ...
    url = "http://gs-prod-bg-web.bio-iad.ea.com/render/?format=json&target=nonNegativeDerivative(sumSeriesWithWildcards(aliasByNode(eadp.gos.blaze.prod.madden-2020-ps4.*.logger.{database,slivers,user,redis}.{ERR,FAIL})))"
...
if 'None' not in ENDPOINTDATA:
    if float(ENDPOINTDATA[0]) < float(TRESHOLD) and float(ENDPOINTDATA[1]) < float(TRESHOLD):
        plugin_exit(STATUS_OK, 'madden-2020-ps4 Errors number below treshold', TRESHOLD)       
    elif float(ENDPOINTDATA[0]) > float(TRESHOLD) or float(ENDPOINTDATA[1]) > float(TRESHOLD):
        notifyissues(sc)
        plugin_exit(STATUS_CRIT, 'madden-2020-ps4 Errors number is above treshold', TRESHOLD)
    elif all("None" in ENDPOINTDATA for x in ENDPOINTDATA): 
        notifyFlatbehavior(sc)
        plugin_exit(STATUS_WARN, 'madden-2020-ps4 data is flat with None Values', '')
