...
def notifyissues(sc):
    msg_issues = "!!ALERT!! fifa-2020-xone errors in the {database,system,slivers,user,redis} blaze components are incremeting above the treshold !!ALERT!!"
...
def notifyFlatbehavior(sc):
    msg_issues = "!!WARNING!! fifa-2020-xone Errors is flat with None values !!WARNING!!"
    sc.api_call('chat.postMessage', channel=CHANNEL, text=msg_issues, username='log_error_alert', icon_emoji=':siren:')
...
#    url = "http://gs-prod-bg-web.bio-iad.ea.com/render/?format=json&target=nonNegativeDerivative(sumSeriesWithWildcards(aliasByNode(eadp.gos.blaze.prod.fifa-2020-ps4.*.logger.*.{ERR,FAIL})))"

    url = "http://gs-prod-bg-web.bio-iad.ea.com/render/?format=json&target=nonNegativeDerivative(sumSeriesWithWildcards(aliasByNode(eadp.gos.blaze.prod.fifa-2020-xone.*.logger.{database,slivers,redis}.{ERR,FAIL})))"
...
if 'null' not in ENDPOINTDATA:
    if float(ENDPOINTDATA[0]) < float(TRESHOLD) and float(ENDPOINTDATA[1]) < float(TRESHOLD):
        plugin_exit(STATUS_OK, 'Fifa-2020-xone Errors number is below treshold', TRESHOLD)       
    elif float(ENDPOINTDATA[0]) > float(TRESHOLD) or float(ENDPOINTDATA[1]) > float(TRESHOLD):
        notifyissues(sc)
        plugin_exit(STATUS_CRIT, 'Fifa-2020-xone Errors number is above treshold', TRESHOLD)
    elif all("None" in ENDPOINTDATA for x in ENDPOINTDATA): 
        notifyFlatbehavior(sc)
        plugin_exit(STATUS_WARN, 'FIFA-2020-XONE Error Number Rate is flat with None Values', '')
