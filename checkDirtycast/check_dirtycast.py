...
def gettotalsumbyregions(resumeregionsdict):
    sumlist = []
    for x in resumeregionsdict.iteritems():
        sumlist.append(x[1][0])   

    return(sum(sumlist))

def getresumeregions(dictallRegions):
    regionsdictresume = {}
    sum = 0
    for h in range(0, len(dictallRegions.items()[0][1])):
        sum = 0
        for x in dictallRegions.values():
            if sum == 0:
                keyregion =  x[h][0]
                regionsdictresume.setdefault(keyregion, [])
            sum = sum + int(x[h][1])
        regionsdictresume[keyregion].append(sum)
    return(regionsdictresume)    

def getRegionbyinstance(status, blazever):
    blazev = validateblaze(blazever)
    if (int(blazev[0]) > 15) or (int(blazev[3]) < 7):
        plugin_exit(STATUS_CRIT, 'Feature not supported by this blaze version', blazever)   

    try:
        regions = ["GaugeUS_aws-bom_Slave", "GaugeUS_aws-brz_Slave", "GaugeUS_aws-cdg_Slave", "GaugeUS_aws-dub_Slave", "GaugeUS_aws-fra_Slave", "GaugeUS_aws-iad_Slave", "GaugeUS_aws-nrt_Slave", "GaugeUS_aws-pdx_Slave", "GaugeUS_aws-sin_Slave", "GaugeUS_aws-syd_Slave", "GaugeUS_ea-ams_Slave", "GaugeUS_m3d-dxb_Slave", "GaugeUS_m3d-hkg_Slave", "GaugeUS_m3d-jnb_Slave", "GaugeUS_mpl-mex_Slave", "GaugeUS_bio-dub_Slave", "GaugeUS_bio-iad_Slave", "GaugeUS_bio-sjc_Slave", "GaugeUS_aws-arn_Slave", "GaugeUS_aws-cmh_Slave", "GaugeUS_aws-yyz_Slave"]
        reqregions = requests.get(status, timeout=5)
        dataregions = etree.fromstring(reqregions.content)
        listaentrada = []
        dict1 = {}
        for instance in dataregions.findall('.//instancename'):
             if "coreSlave" in instance.text:
                 dict1.setdefault(instance.text, [])
                 for item in dataregions.findall('.//components/componentstatus/info/entry'):
                     if item.attrib.values()[0] in regions:
#                        print(item.attrib.values()[0], item.text)
                        listaentrada = [item.attrib.values()[0].split("_")[1], int(item.text)]
                        dict1[instance.text].append(listaentrada)
        return(dict1)
...
def getinstancesperboxredirector(allinstances, allhosts):
    boxestimes = []
    boxesinstances = []
#    print("All instances: ", allinstances)
#    print("All hosts: ", allhosts)
    ...
    print("Boxes Instances: ", boxesinstances)
...
def searchredirector2(schurl, sername, env, titledata, cl, cpu, cfgs, gauge_ins, region):
...
#                 print("Status Page :", status)
                 regiondict = getRegionbyinstance(status, blazever)     
#                 print(type(regiondict)) 
#                 print("Region dictionary: ", regiondict)
                 dictallRegions.update(regiondict)
                 print(dictallRegions)
#                 resumeregionsdict = getresumeregions(dictallRegions)
                 table.field_names = ["Instance", "Serv", "Int IP", "Port", "Dom Name", "Ext IP"]
#                 print(resumeregionsdict)
        else:
            table.field_names = ["Instance", "Serv", "Int IP", "Port", "Dom Name", "Ext IP"]
    if region:
        resumeregionsdict = getresumeregions(dictallRegions)
        sumconnections = gettotalsumbyregions(resumeregionsdict)      
...
## Table and display of coreslave ips
    ...
    if region:
        print("Users connected from regions: ")
        print(resumeregionsdict)
        print("Users Connected to the game: ", sumconnections)
    if gauge_ins:
...
def main(argv):
    ...
    parser.add_option('-l', '--cpu', dest='cpu',
                      help='Show instances information, type 1 to activate', default=0)
    parser.add_option('-g', '--reg', dest='reg',
                      help='Show connections by region -only for blaze 1.60 and above-, type 1 to activate', default=0)
