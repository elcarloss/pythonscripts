...
for jid,item in datas.iteritems():
	...
	for ep in f:
		...
		if cfg_result:
			...
			rid += 1
			final_report[rid] = {"ServiceName": str(item).translate(None, "'u"), "Branch": jid, "File": cfg_result[0]['depotFile'], "Endpoint": ep}
		if boot_result:
			print "Found: " + ep + " in " + boot_result[0]['depotFile']
			rid += 1
			final_report[rid] = {"ServiceName": str(item).translate(None, "'u"), "Branch": jid, "File": boot_result[0]['depotFile'], "Endpoint": ep}
