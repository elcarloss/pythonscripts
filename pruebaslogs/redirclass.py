...
class redir(object):
    ...
    def get_instanceslistwithports(self):
        ...
        for serverinstance in self.data.findall('.//serverinstance'):
            ...
            inservice = serverinstance.find('inservice').text
            ipnum = serverinstance.findall('.//valu')[0].find('ip').text
            for serverendpointinfo in serverinstance.findall('.//serverendpointinfo'):
                if serverendpointinfo.find('protocol').text == 'httpxml' and serverendpointinfo.find('bindtype').text == '1':
                    port = serverendpointinfo.find('.//port').text
            ip = socket.inet_ntoa(struct.pack('!L', int(ipnum)))
            instanceinfo.extend([instancename, inservice, ip, port])
            allinstances.append(instanceinfo)
        return allinstances
