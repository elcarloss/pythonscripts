...
KEYINSTANCENAMEAUXLSTB15 = (
    [
        'search', 'gr', 'cmrAux', 'search', 'mm', 'osdkAux', 'osdk', 
        'maut', 'mutAux',
        'custom', 'onlinecareer', 'nhlAux', 'odtAux', 'nba', 'anarchists',
        'realtime', 'spl', 'chl', 'item', 'autolog', 'kickback', 'competitive',
        'nonZdt', 'rsp', 'madAux', 'cmwAux', 'da3Aux','mehAux','cmrAux'
    ]
)

KEYINSTANCENAMEMASTERLSTB15 = (
    [
        'config', 'core', 'coreNS', 'rsp', 'custom', 'osdkAux', 'osdk', 'nba',
        'cmrAux', 'aux', 'chl', 'spi', 'item', 'kick', 'spl', 'competitive',
        'nonZdt', 'onlinecareer', 'odtAux', 'madAux', 'cmwAux', 'da3Aux','mutAux',
        'nhlAux'
    ]
)
...
def validatelenxml(reqcontent):
    ...
    """
    if len(reqcontent) < XMLBADLENGHT:
        plugin_exit(STATUS_WARN, "The xml does not appear to have any style information " +
...
def cutstring(insargument, instanceargumentmaster):
    ...
    print("Indexkeyword master cutstring: ", indexkeywordm)
    print("Indexkeywordvarios cutstring: ", indexkeywords)
...
def validateinserviceinstances(xmlredir, instanceargument):
    ...
    inservice 0 (Stuck)
    """
    print("Nombre de la Instancia: ", instanceargument)
    instanceargumentmaster = "temp"
    instancekeywords, instancekeywordm = cutstring(instanceargument, instanceargumentmaster)
    print("Instance Slave: ",instancekeywords)
    print("Instance Master: ", instancekeywordm)
    if instancekeywords in KEYINSTANCENAMEAUXLSTB15:
        ...
        print("Entro a if de instancekeywordm con el nombre de :", instancekeywordm)
...
def searchinchredirector(schurl, sername, insargument, decip):
    ...
    try:
        ...
        plugin_exit(STATUS_UNK, 'Failed to get the redirector xml', err)
    ...
    if all([insargument in reqredirectorurl.content, sername in reqredirectorurl.content,
        ...
        plugin_exit(STATUS_OK, "The process is registered in Redirector", "")
              ...
              validateinserviceinstances(xmlredirector, insargument):
        plugin_exit(STATUS_OK, "The process is registered in Redirector", "")
    else:
...
def searchingetstatusxml(urldata, iargument, sname):
    ...
    try:
        ...
        plugin_exit(STATUS_CRIT, "Servicename or instance not registered on Redirector -" +
                    "Failed to get the getstatus xml", err)
    ...
    if bversion in BLAZEVERSIONS:
        ...
        plugin_exit(STATUS_WARN, "This version check only suport blaze 13,14 and 15", "")
