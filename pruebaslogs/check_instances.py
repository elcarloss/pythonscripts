...
def filep4sync(branch, platform, env):
    p4 = P4()
...
def searchredirector2(schurl, sername, env, titledata, cl, cpu):
    ...
    filep4sync(branch, platform, env)
    ...
    instotal = []
    extip = []
    print "Processing, please wait - Please type your P4 password (or enter if you are already logged on P4) - "
    current = check_current(sername)
...
#        instances_host_table.add_row("%s: %i" % (key, dictcfgfile[key]))
    ...
    print("\n\033[1;32;40mHosts used in this title: " +str(len(allhosts)))
